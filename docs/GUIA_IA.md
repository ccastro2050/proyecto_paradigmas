...
> `uvicorn main:app --port 8006 --reload` (arrancar la API),
> `python pruebas\prueba_capas.py` (la prueba de capas) y los `curl` del
> smoke test (`curl http://localhost:8006/api/producto`). En el chat, la IA
...
| `uvicorn main:app --port 8006 --reload` · `python pruebas\prueba_capas.py` | `api_facturas\` (ahí vive el código) — primero `cd api_facturas` |
...
7. En mi máquina TAMBIÉN corre el proyecto clonado del curso con sus
   ...
   a. Publica los puertos del host con +100: en el docker-compose.yml la
      API va "8106:8006" y PostgreSQL va "15536:5432" (adentro de los
   ...
   Docker) apunta a localhost:15536. Cuando me des URLs o comandos de
   prueba, usa localhost:8106 (API) y localhost:15536 (BD).
...
3. **El punto de control real es el smoke test final** (7_quickstart.md):
   ...
   (regla 7 del prompt) — donde el quickstart diga `localhost:8006` use
   `localhost:8106`, y donde diga `15436` use `15536`.
