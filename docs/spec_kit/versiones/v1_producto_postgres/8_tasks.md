...
- [ ] Crear el `docker-compose.yml` con el servicio `postgres` (volumen
      `pgdata`, `db/init.sql` montado, puerto 15436, healthcheck — ver
...
**Verificar:** `uvicorn main:app --port 8006 --reload` y en `/docs` probar:
...
- [ ] Agregar al `docker-compose.yml` el servicio `api-facturas`: `build:`,
      código montado como volumen + `--reload`, puerto 8006, `DB_POSTGRES`
