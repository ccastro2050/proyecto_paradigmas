...
| `EXPOSE 8006` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15436 de su PC (`-p 15436:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8006"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8006"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15436"| POSTGRES
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15436:5432"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-facturas:
    ...
    ports:
      - "8006:8006"                # http://localhost:8006/docs
    environment:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8006`, `localhost:15436`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas_default --restart unless-stopped `
  -e "POSTGRES_DB=bdfacturas_postgres_local" `
  -e "POSTGRES_USER=paradigmas" `
  -e "POSTGRES_PASSWORD=paradigmas123" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15436:5432 postgres:16-alpine
...
docker run -d --name api-facturas --network proyecto_paradigmas_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/bdfacturas_postgres_local" `
  -v "${PWD}/api_facturas:/app" `
  -p 8006:8006 api-facturas uvicorn main:app --host 0.0.0.0 --port 8006 --reload
...
```yaml
URL_API: http://api-facturas:8006
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8006` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8006` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8006:8006`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8006`, el front
...
```powershell
docker network inspect proyecto_paradigmas_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas` produce
la red `proyecto_paradigmas_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas_default has active endpoints (name:"proyecto_paradigmas-api-facturas-1" …)
```
