...
http://localhost:8006/docs** (FastAPI la genera sola). Esta colección es el
...
La colección usa la variable `base` = `http://localhost:8006` (el proyecto
del curso). Si está probando **SU reconstrucción** (la de la
[GUIA_IA](../docs/GUIA_IA.md), que corre en el puerto 8106): clic en la
colección → pestaña **Variables** → cambie `base` a
`http://localhost:8106`. Una sola edición y las 13 peticiones apuntan a su
proyecto.
